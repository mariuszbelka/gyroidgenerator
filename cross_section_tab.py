...
class CrossSectionTab(QWidget):
    ...
    def _update_view(self):
        ...
        # Plot vector path
        path_2d = slice_data['path']
        for entity in path_2d.entities:
            disc = entity.discrete(path_2d.vertices)
            ax.plot(disc[:, 0], disc[:, 1], 'k-', linewidth=1)
            ax.fill(disc[:, 0], disc[:, 1], 'k', alpha=0.7)
        ...
        self.info_label.setText(
            ...
            f"Solid Area: {slice_data['solid_area']:.3f} mm²\n"
            f"Method: Mesh Slicing (Vector)\n"
