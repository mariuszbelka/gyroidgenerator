...
from predictions import (
    SOLVENT_PRESETS, ANALYTE_PRESETS,
...
class PredictionsTab(QWidget):
    ...
    def _init_ui(self):
        ...
        self.d_polymer_spin.setRange(0.001, 1000.0)
    ...
    def _get_d_polymer(self) -> float:
        return self.d_polymer_spin.value() * 1e-11
    ...
    def _calc_pressure_drop(self):
        ...
        result = calc_pressure_drop(
            porosity, d_h_m, col_L, viscosity, flow, col_d
        )
        self._results['dp'] = result

        # Curve
        curve = calc_pressure_drop_curve(
            porosity, d_h_m, col_L, viscosity, col_d,
        ...
        ax.set_title('Pressure Drop vs Flow Rate (Kozeny-Carman)')
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ΔP = {result['delta_P_bar']:.2f} bar  ({result['delta_P_psi']:.0f} PSI)
  Flow: {flow:.1f} µL/min
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  N/m = {result['N_per_m']:.0f} plates/m
...
"""
    ...
    def _calc_desorption(self):
        ...
        # Mean desorption path — use from v2 stats if available,
        # otherwise estimate from geometry
        stats = self._basic_stats
        mean_path = stats.get('mean_desorption_path_mm', None)
        if mean_path is None:
            # Estimate: R/4 × τ for sphere, L/6 × τ for cylinder
            if self._geometry_params.shape == 'sphere':
                R = self._geometry_params.dimensions.get('diameter', 2.0) / 2
                mean_path = R / 4 * 1.41
            else:
                L = self._geometry_params.dimensions.get('height', 50)
                mean_path = L / 6 * 1.41
        ...
        result = calc_desorption_time(
            ...
            D_polymer=self._get_d_polymer(),
            D_mobile=self._get_d_mobile(),
            mean_desorption_path_m=mean_path / 1000.0,
        )
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  Wall/Channel ratio: {result['t_wall_to_channel_ratio']:.1f}×
"""
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Total capacity: {result['total_capacity_mg']*1000:.2f} µg  ({result['total_capacity_mg']:.4f} mg)

  Volumetric:   {result['Q_volumetric_mg_mL']:.3f} mg/mL
  Gravimetric:  {result['Q_gravimetric_mg_g']:.3f} mg/g
