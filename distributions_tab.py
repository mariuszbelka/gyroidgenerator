        ...
        for name in self.analyses:
            try:
                ...
                if name == 'wall_thickness':
                    result = self.analyzer.calc_wall_thickness_distribution(self.voxels_per_uc)
                elif name == 'channel_width':
                    result = self.analyzer.calc_channel_width_distribution(self.voxels_per_uc)
                    ...
                    result = self.analyzer.calc_throat_distribution(self.voxels_per_uc)
                elif name == 'printability':
                    result = self.analyzer.calc_printability(self.voxels_per_uc)
                else:
                    continue
...
class DistributionsTab(QWidget):
    ...
    def _init_ui(self):
        ...
        self.result_tabs.addTab(self.conn_text, "Connectivity")
        ...
        self.result_tabs.addTab(self.asa_widget, "ASA")
    ...
    def _display_wall_thickness(self, r):
        ...
        ax.set_ylabel('Count')
        ax.set_title('Wall Thickness Distribution (Hildebrand method)')
...
"""
    ...
    def _display_asa(self, r):
        ...
        radii = [p['probe_radius_nm'] for p in profiles]
        ...
        ax.set_title('Accessible Surface Area vs Probe Size')
        ax.set_ylim(0, 105)
        ax.grid(True, alpha=0.3)
        ...
        text += "Probe [nm]  │  ASA [mm²]     │  Fraction\n"
        text += "────────────┼────────────────┼──────────\n"
        for p in profiles:
            text += f"  {p['probe_radius_nm']:6.1f}     │  {p['accessible_SA_mm2']:10.2f}   │  {p['fraction']:.1%}\n"
        text += f"\nTime: {r['calculation_time']:.1f}s"
    ...
    def _display_throat(self, r):
        ...
        ax.axvline(r['p10_um'], color='red', linestyle='--', linewidth=2,
                   label=f"P10={r['p10_um']:.0f} µm (critical!)")
        ax.axvline(r['p50_um'], color='green', linestyle='-', linewidth=2,
                   label=f"P50={r['p50_um']:.0f} µm")

        ax.set_xlabel('Throat Diameter [µm]')
        ax.set_ylabel('Count')
        ax.set_title('Throat / Constriction Distribution')
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
        self.throat_figure.tight_layout()
        ...
        text = (
            ...
            f"Min: {r['min_um']:.1f} µm | Max: {r['max_um']:.1f} µm\n"
            f"P10: {r['p10_um']:.1f} µm | P50: {r['p50_um']:.1f} µm | P90: {r['p90_um']:.1f} µm\n"
            f"Throat/Pore ratio: {r['throat_to_pore_ratio']:.2f} | "
            f"Throats found: {r['n_throats']:,} | Time: {r['calculation_time']:.1f}s"
        )
        for w in r.get('warnings', []):
            text += f"\n⚠ {w}"
