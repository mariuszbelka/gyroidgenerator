...
class StatisticsTabV2(QWidget):
    ...
    def _create_instant_calcs_group(self) -> QGroupBox:
        ...
        self.cb_dist_theoretical = QCheckBox("Distance to Edge (theoretical)")
        self.cb_dist_theoretical.setChecked(True)
        
        self.cb_tortuosity = QCheckBox("Tortuosity Factor")
        self.cb_tortuosity.setChecked(True)
        
        self.cb_mass = QCheckBox("Mass (g)")
        self.cb_mass.setChecked(True)
        
        self.cb_external_area = QCheckBox("External Surface Area")
        ...
        layout.addWidget(self.cb_dist_theoretical)
        layout.addWidget(self.cb_tortuosity)
    ...
    def _create_accurate_calcs_group(self) -> QGroupBox:
        ...
        desorption_label = QLabel("<b>Desorption Path Analysis:</b>")
        layout.addWidget(desorption_label)
        
        self.cb_desorption_fast = QCheckBox("Fast Approximation (Euclidean + τ)")
        self.cb_desorption_fast.setChecked(False)  # Default OFF
        fast_info = QLabel("  → ~5-10 seconds, ±15-20% accuracy")
    ...
    def _set_all_instant(self, checked: bool):
        ...
        self.cb_dist_theoretical.setChecked(checked)
        self.cb_tortuosity.setChecked(checked)
    ...
    def calculate_selected(self):
        ...
        if self.cb_dist_theoretical.isChecked():
            calcs.append(('Distance to Edge (Theoretical)', 
                         self.stats_analyzer.calc_distance_to_edge_theoretical, 
                         {}))
        
        if self.cb_tortuosity.isChecked():
            calcs.append(('Tortuosity', 
                         self.stats_analyzer.calc_tortuosity, 
                         {}))
    ...
    def display_results(self):
        ...
        for name, result in self.results.items():
            ...
            elif 'mean_distance_mm' in result:
                text += f"  Mean: {result['mean_distance_mm']:.2f} mm\n"
                text += f"  Method: {result['method']}\n"
                if 'accuracy' in result:
                    text += f"  Accuracy: {result['accuracy']}\n"
                text += f"  ⏱ Time: {result['calculation_time']:.3f}s\n\n"
