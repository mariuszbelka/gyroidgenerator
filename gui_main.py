...
class MainWindow(QMainWindow):
    ...
    def create_control_panel(self) -> QWidget:
        ...
        h_btn_layout.addWidget(self.export_btn)

        self.import_btn = QPushButton("Import STL...")
        self.import_btn.clicked.connect(self.on_import_clicked)
        h_btn_layout.addWidget(self.import_btn)
    ...
    def on_import_clicked(self):
        ...
        try:
            ...
            # Create a dummy generator/params for imported mesh
            self.current_mesh = mesh

            # Estimate parameters or use current UI values
            unit_cell = self.gyroid_group.get_value('unit_cell')
            wall_thickness = self.gyroid_group.get_value('wall_thickness')
            shape = self.shape_selector.currentText().lower()

            # Trigger "finished" logic to update all tabs
            result = {
                'mesh': mesh,
                'generator': None, # Will handle in update logic
                'unit_cell': unit_cell,
                'wall_thickness': wall_thickness,
                'threshold': 0.0,
                'shape': shape,
            ...
            # Since we don't have a generator, we need to adapt on_generation_finished
            self._handle_imported_mesh(result)

            QMessageBox.information(self, "Import Successful", f"Loaded: {os.path.basename(filename)}")

        except Exception as e:
            QMessageBox.critical(self, "Import Error", f"Failed to load STL:\n\n{str(e)}")
            logger.error(f"Import error: {str(e)}")

    def _handle_imported_mesh(self, result: dict):
        """Specially handle imported mesh to update tabs."""
        self.current_mesh = result['mesh']
        ...
        # Prepare parameters for tabs
        shape = result['shape']
        unit_cell = result['unit_cell']
        wall_thickness = result['wall_thickness']

        # Container dims from mesh bounds
        bounds = self.current_mesh.bounds
        size = bounds[1] - bounds[0]
        dimensions = {'diameter': size[0], 'height': size[2]}

        self.current_geometry_params = GeometryParams(shape=shape, dimensions=dimensions)
        self.current_gyroid_params = GyroidParams(unit_cell=unit_cell, wall_thickness=wall_thickness)
        ...
        self.predictions_tab.set_data(
            ...
            geometry_params=self.current_geometry_params,
            gyroid_params=self.current_gyroid_params,
            basic_stats=stats_analyzer.get_basic_stats()
        )
    ...
    def on_save_to_compare(self):
        ...
        if self.current_generator:
            ...
            # For imported meshes, get what we can from StatisticsV2
            stats_analyzer = StatisticsV2(
                mesh=self.current_mesh,
                geometry=self.current_geometry_params,
                gyroid_params=self.current_gyroid_params
            )
            bs = stats_analyzer.get_basic_stats()
            stats_dict['porosity'] = bs.get('porosity')
            stats_dict['surface_area'] = bs.get('surface_area')
            stats_dict['volume'] = bs.get('gyroid_volume')
